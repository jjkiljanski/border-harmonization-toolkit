...
with open('initial_state.json', 'r', encoding='utf-8') as f:
    initial_state = json.load(f)

adm_state_dict = []
adm_state_dict = {"HOMELAND": {}, "ABROAD": {}}
dist_state_list = []
region_state_list = []
...
for region_name, dist_list in initial_state.items():
    if region_name in ["ZIEMIA WILEŃSKA", "ŚLĄSK NIEMIECKI", "ZAOLZIE"]:
        ...
        country_name = "HOMELAND"
    for district in dist_list:
        adm_state_dict.setdefault(country_name, {})
        adm_state_dict[country_name].setdefault(region_name, {})
        adm_state_dict[country_name][region_name][district['district_name']] = {} # Administrative state
    ...
    region_state_list.append({"name_id": region_name, "name_variants": [region_name], "is_homeland": True, "states": [{"current_name": region_name, "timespan": None}]})
